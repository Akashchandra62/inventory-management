...
def _generate_pdf(invoice: dict, path: str):
    ...
    s_shopname  = S('sn', fontSize=20, alignment=TA_CENTER, fontName='Helvetica-Bold',
                    textColor=colors.HexColor('#8B0000'), spaceAfter=1)
    s_tagline   = S('tl', fontSize=9,  alignment=TA_CENTER, fontName='Helvetica-Oblique', spaceAfter=1)
    s_address   = S('ad', fontSize=8,  alignment=TA_CENTER, spaceAfter=1)
    s_mobile    = S('mb', fontSize=8,  alignment=TA_CENTER, fontName='Helvetica-Bold')
    ...
    # ── HEADER ROW: Logo | Shop Info | Logo (mirror) ─────────
    header_content = []

    # Left: Logo image (if uploaded)
    if os.path.exists(LOGO_FILE):
        try:
            logo_img = RLImage(LOGO_FILE, width=22*mm, height=22*mm)
            header_content.append(logo_img)
        except Exception:
            header_content.append(Paragraph("", s_normal))
    else:
        header_content.append(Paragraph("", s_normal))

    # Center: shop name, tagline, address, mobile
    center_items = [
        Paragraph("|| श्री गणेशाय नमः ||", s_ganesh),
        Paragraph(shop_name, s_shopname),
    ]
    if tagline:
        center_items.append(Paragraph(tagline, s_tagline))
    if address:
        center_items.append(Paragraph(address, s_address))

    mob_line = f"Mobile No.: {mobile}"
    if mobile2:
        mob_line += f"  &nbsp;&nbsp;  {mobile2}"
    center_items.append(Paragraph(mob_line, s_mobile))

    from reportlab.platypus import KeepInFrame
    center_frame = KeepInFrame(130*mm, 50*mm, center_items)
    header_content.append(center_frame)

    # Right: Logo again (right side, like the original) or blank
    if os.path.exists(LOGO_FILE):
        try:
            logo_img2 = RLImage(LOGO_FILE, width=22*mm, height=22*mm)
            header_content.append(logo_img2)
        except Exception:
            header_content.append(Paragraph("", s_normal))
    else:
        header_content.append(Paragraph("", s_normal))

    header_tbl = Table([header_content], colWidths=[25*mm, 140*mm, 25*mm])
    ...
    story.append(HRFlowable(width=W, thickness=1.5, color=colors.black, spaceBefore=2, spaceAfter=2))
    ...
    for i, item in enumerate(items, 1):
        ...
        making_str = f"{making:.2f}%" if making <= 100 else f"₹{making:.2f}"
    ...
    # ── BOTTOM SECTION: Payment | Totals | QR ────────────────

    # Left: Payment Details — label left, value right aligned
    pay_rows = [
    ...
    bottom_row = [[pay_tbl, totals_tbl]]
    bottom_tbl = Table(bottom_row, colWidths=[95*mm, 95*mm])
    ...
    # QR code only — no label text
    if os.path.exists(QR_FILE):
        try:
            qr_img_bank  = RLImage(QR_FILE, width=30*mm, height=30*mm)
            qr_bank_cell = [qr_img_bank]
        except Exception:
            qr_bank_cell = [Paragraph("", s_normal)]
    else:
        qr_bank_cell = [Paragraph("", s_normal)]
    ...
    sign_row = [['', Paragraph("Sign & Seal", S('ss', fontSize=8, alignment=TA_RIGHT, fontName='Helvetica-Bold'))]]
    sign_tbl = Table(sign_row, colWidths=[W * 0.7, W * 0.3])
